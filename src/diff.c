                        ...
                        int *df_arr_col_size, const int *outmap)
{
  ...
  diffcomparisonpath_flat_T *diffcomparisonpath_flat = ALLOC_MULT(diffcomparisonpath_flat_T, memsize);
  // allocate memory here
  for (int i = 0; i < memsize; i++) {
    diffcomparisonpath_flat[i].df_decision = ALLOC_MULT(int, memsize_decisions);
  }
